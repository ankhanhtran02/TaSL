...
def get_task_data_dict(task, tokenizer, batch_size, splits=['train', 'validation', 'test']):
    ''' return:
    - tasks_data_dict: {'train':Dataset, 'validation':Dataset, 'test':Dataset})
    '''
    task_data_dict = {}
    for split in splits:
        if split == 'train':
            ...
            k = 5000
        task_data_dict[split] = T5Dataset(tokenizer, task).get_final_ds(task=task, split=split, k=k, batch_size=batch_size)
        print(f"Loaded {split} split for task {task} with {len(task_data_dict[split])} samples.")
...
if __name__ == "__main__":
    ...
    ds = T5Dataset(tokenizer, 'KodCode').get_final_ds(task='KodCode', split='train', batch_size=32)
    print(len(ds))
    print(next(iter(ds)))